...
class OrganisationSettingsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // --- Replicar lógica del botón Guardar para aplicar idioma globalmente ---
        // Actualizar locale en empresa principal
        $mainCompany = Company::first();
        if ($mainCompany) {
            $mainCompany->locale = 'es';
            $mainCompany->save();
        }

        // Actualizar locale en usuario principal (si existe)
        $mainUser = \App\Models\User::first();
        if ($mainUser) {
            $mainUser->locale = 'es';
            $mainUser->saveQuietly();
        }

        // Actualizar locale en globalSetting
