...
class Deal extends BaseModel
{
    ...
    /**
     * Verifica si el deal está en la penúltima etapa del pipeline
     */
    public function isInPenultimateStage(): bool
    {
        if (!$this->lead_pipeline_id || !$this->pipeline_stage_id) {
            return false;
        }

        $stages = PipelineStage::where('lead_pipeline_id', $this->lead_pipeline_id)
        ...
        $penultimateStage = $stages[$stages->count() - 2];
        return $this->pipeline_stage_id == $penultimateStage->id;
    }

    /**
     * Verifica si hay otros deals con el mismo inmueble en el mismo pipeline
     * y si alguno de ellos está en la penúltima etapa
     */
    public function hasDuplicateInPenultimateStage(): bool
    {
        if (!$this->lead_pipeline_id || $this->products->isEmpty()) {
            return false;
        }

        $productIds = $this->products->pluck('id')->toArray();

        // Buscar otros deals en el mismo pipeline con los mismos productos
        $otherDealsInPenultimate = Deal::where('id', '!=', $this->id)
            ->where('lead_pipeline_id', $this->lead_pipeline_id)
            ->whereHas('products', function($query) use ($productIds) {
                $query->whereIn('products.id', $productIds);
            })
            ->get()
            ->filter(function($deal) {
                return $deal->isInPenultimateStage();
            });

        return $otherDealsInPenultimate->isNotEmpty();
    }
